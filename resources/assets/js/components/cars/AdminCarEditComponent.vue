<template>
  <div>
    <admin-navbar title="Car Edit">
      <v-divider vertical></v-divider>
      <v-btn text :to="{ name: 'admin_cars_list' }" exact>Car List</v-btn>
      <v-divider vertical></v-divider>
      <v-btn text :to="{ name: 'admin_car_add' }" exact>Car Add</v-btn>
      <v-divider vertical></v-divider>
    </admin-navbar>
    <v-container>
      <v-row>
        <v-col>
          <v-card>
            <v-card-title>
              <div class="subtitle-1 primary--text">Edit Car</div>
              <v-spacer></v-spacer>
...
export default {
  ...
  created() {
    ...
    this.$store.dispatch("cars/detail", { id: this.$route.params.id });
  },
  computed: {
    status_array() {
    ...
    car() {
      return this.$store.getters["cars/get_car"];
    }
  },
  watch: {
    car(car) {
      this.vehicle_model = car.vehicle_model
      this.vehicle_number = car.vehicle_number
      this.seating_capacity = car.seating_capacity
      this.rent_per_day = car.rent_per_day
  ...
  methods: {
    submit() {
      if (this.$refs.form.validate()) {
        ...
        this.$store.dispatch("cars/edit", {id, formdata});
      }
    },
  },
};
